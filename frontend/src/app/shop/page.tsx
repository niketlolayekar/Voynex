...
const CATEGORY_EMOJI: Record<string, string> = { jackets: '🧥', bags: '🎒', accessories: '🕶️', gear: '⛺', electronics: '📱' };
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..."
              className="w-full pl-12 pr-4 py-3 rounded-xl border border-border bg-card text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 tap-target" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-4 py-2.5 rounded-xl text-sm font-medium capitalize whitespace-nowrap transition-all tap-target ${
                  category === c ? 'bg-accent text-white' : 'bg-card border border-border hover:bg-surface-hover'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filtered.map((product, idx) => (
            <motion.div key={product._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
              ...
              <div className="h-52 bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700/50 dark:to-gray-800/50 relative overflow-hidden img-dark-overlay">
                <div className="absolute inset-0 flex items-center justify-center text-5xl opacity-20">
                  {CATEGORY_EMOJI[product.category] || '🎒'}
                </div>
                {product.stock < 20 && (
                  <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full bg-orange text-white text-[10px] font-bold">Low Stock</span>
                )}
              </div>
              <div className="p-5">
                <p className="text-xs text-muted capitalize mb-1">{product.category}</p>
                <h3 className="font-semibold text-sm mb-2 group-hover:text-accent transition-colors line-clamp-1">{product.name}</h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => handleAdd(product)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all tap-target ${
                      addedId === product._id ? 'bg-accent text-white' : 'bg-accent/10 text-accent hover:bg-accent hover:text-white'}`}>
                    <FiShoppingCart size={14} /> {addedId === product._id ? 'Added!' : 'Add to Cart'}
                  </button>
                </div>
              </div>
            </motion.div>
